	...
	<v-expansion-panel hide-actions>
		<v-expansion-panel-header>
			<v-row align="center" justify="center" no-gutters>
				...
				<v-col
					><strong>{{ dog.name }}</strong></v-col
				>

				<v-col>{{ dog.breed }}</v-col>
				<v-col>{{ displayAge(dog) }}</v-col>
			</v-row>
		</v-expansion-panel-header>
		...
		<v-expansion-panel-content class="py-0 my-0">
			...
			<v-row class="d-flex justify-space-around">
				<v-btn color="info" class="mt-3" @click="$emit('edit', dog)">Edit</v-btn>
		...
		methods: {
			removeDog() {
				...
				axios
					...
					.then(this.$store.commit("removeDog", this.dog))
